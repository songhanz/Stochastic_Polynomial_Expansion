...
def parse_data_hyper(dataset, url, n_hidden_layer):
    ...
    hypers = {
        ...
        "batch_size": 128, #round(datapoint ** 0.5), 
        "learning_rate": 0.005,
        "lambda": 1.0,
...
def update_adapter(data, hypers):
    # this adapter serves as output denormalization layers
...
for dataset, url in progress_bar:
    ...
    for n_hidden_layer in range(4, 5):

        X, y, hypers, n_repeat = parse_data_hyper(dataset, url, n_hidden_layer)

        save_dir  = 'UCI_results_20valid/' + dataset + '_' + nonlinearity + '_' + str(n_hidden_layer) + 'layer_adapter/'
