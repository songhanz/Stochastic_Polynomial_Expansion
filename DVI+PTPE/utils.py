...
def run_one_epoch(sess, data, model, metrics, train_op, hypers, dynamic_hypers):
    ...
    count = 0
    running_accuracy = running_logprob = running_loglikelihood = 0
    for batch in batched(data, hypers):
        ...
        running_logprob = \
            ...
            / new_count
        running_loglikelihood = \
            (count * running_loglikelihood + len(batch[0]) * result[0]['loglikelihood']) \
            / new_count
        ...
        result[0].update({'count': len(batch[0]),
                          ...
                          'running_loglikelihood': running_loglikelihood})
...
def train_valid_test(data, sess, model_and_metrics, train_op, hypers, verbose=True):
    ...
    for section in hypers['sections_to_run']:
        ...
        if verbose:
            ...
            print(' %s RMSE = %.4f | logL (Bayes) = %.4f | logL (point) = %.4f' % (section, accuracies[section], 
                                                          summary[section][-1][-1]['running_logprob'],
                                                          summary[section][-1][-1]['running_loglikelihood']))
    if verbose:
        print()
