...
"""
<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
Below is the implementation of PTPE 
<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
"""
def linear_relu(x, A, b):
    """
    ...
    This function implement 3rd order PTPE of ReLU. The one above is the original function
    """
    # Extract the diagonal
    x_var_diag = tf.linalg.diag_part(x.var) 
    # Replace values smaller than EPSILON with EPSILON
    x_var_diag = tf.maximum(x_var_diag, EPSILON)
    # this will remove the negative values of variance, this may cause by not strictly positive definite covariance
    x.var      = tf.linalg.set_diag(x.var, x_var_diag)

    sqrt_x_var_diag = tf.sqrt(x_var_diag)
    mu         = x.mean / sqrt_x_var_diag

    B = bu.standard_gaussian(mu) / sqrt_x_var_diag
    C = bu.gaussian_cdf(mu)

    def relu_covariance(x):
        A1 = C
        A2 = B / 2
        A3 = -B * x.mean / x_var_diag / 6

        V1 = x.var
        V2 = 2*tf.pow(x.var, 2)
        V3 = 6*tf.pow(x.var, 3) + 9* tf.expand_dims(x_var_diag, axis=-1) * x.var * tf.expand_dims(x_var_diag, axis=1)

        return tf.expand_dims(A1, axis=-1)   * V1 * tf.expand_dims(A1, axis=1) \
               + tf.expand_dims(A2, axis=-1) * V2 * tf.expand_dims(A2, axis=1) \
               + tf.expand_dims(A3, axis=-1) * V3 * tf.expand_dims(A3, axis=1)
    
    z_mean = x.mean * C + x_var_diag * B
    z_cov  = relu_covariance(x)
    y_mean = tf.matmul(z_mean, A.mean) + b.mean
    y_cov  = linear_covariance(z_mean, z_cov, A, b)
...
def linear_gelu(x, A, b):
    """
    compute y = gelu(x)^T A + b
    """
    # Extract the diagonal
    x_var_diag = tf.linalg.diag_part(x.var) 
    # Replace values smaller than EPSILON with EPSILON
    ...
    x_hat_var_diag = x_var_diag + 1
    sqrt_x_hat_var_diag = tf.sqrt(x_hat_var_diag)

    B = bu.standard_gaussian(x.mean / sqrt_x_hat_var_diag) / sqrt_x_hat_var_diag
    C = bu.gaussian_cdf(x.mean / sqrt_x_hat_var_diag)
    
    def gelu_covariance(x):
        A1 = C + B * x.mean / x_hat_var_diag
        A2 = 1/2 * B * (1 + 1/x_hat_var_diag - tf.pow(x.mean, 2)/ tf.pow(x_hat_var_diag,2) )
        A3 = 1/6 * B * (- x.mean /x_hat_var_diag - 3 * x.mean / tf.pow(x_hat_var_diag,2) + tf.pow(x.mean,3) / tf.pow(x_hat_var_diag,3) )
...
def linear_tanh(x, A, b):
    """
    compute y = tanh(x)^T A + b
    """
    # Extract the diagonal
    x_var_diag = tf.linalg.diag_part(x.var) 
    # Replace values smaller than EPSILON with EPSILON
    ...
    # gamma = tf.constant([0.558267543233651,0.859623142094298,0.859623146088812,1.261245008299549], dtype=tf.float32)
    gamma = tf.constant([0.932509446848035,0.743885472858475,0.711503736783192,0.707185882493931,1.036267327797645,1.041272595334387,0.706179751346938,0.484137541329413,1.053317393402910,1.444605073000763], dtype=tf.float32)

    gamma = tf.expand_dims(gamma, axis=0)
    
    x_hat_var_diag      = tf.expand_dims(x_var_diag, axis=-1) + tf.expand_dims(1/2/tf.pow(gamma,2), axis=1)
    sqrt_x_hat_var_diag = tf.sqrt(x_hat_var_diag)
    mu_hat              = tf.expand_dims(x.mean, axis=-1)/ sqrt_x_hat_var_diag

    B = bu.standard_gaussian(mu_hat) / sqrt_x_hat_var_diag
    C = bu.gaussian_cdf(mu_hat)
    
    def tanh_covariance(x):

        mean_expand = tf.expand_dims(x.mean, axis=-1)

        A1 = 2*B
        A1 = tf.reduce_mean(A1, axis=-1)
        A2 = - B * mean_expand / x_hat_var_diag
        A2 = tf.reduce_mean(A2, axis=-1)
        A3 = 1/3 * B * (tf.pow(mean_expand,2) - x_hat_var_diag) / tf.pow(x_hat_var_diag, 2)
        A3 = tf.reduce_mean(A3, axis=-1)
    ...
    z_mean = tf.reduce_mean(2*C-1, axis=-1)
    z_cov  = tanh_covariance(x)
    y_mean = tf.matmul(z_mean, A.mean) + b.mean
    y_cov  = linear_covariance(z_mean, z_cov, A, b)
    return gv.GaussianVar(y_mean, y_cov)
