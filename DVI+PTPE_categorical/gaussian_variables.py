...
class DiagonalGaussianVar(object):
    ...
    def log_likelihood(self, x):
        ...
        return -0.5 * (log2pi + tf.log(self.var) + (x - self.mean)**2 / self.var)
...
def KL(p, q, hypers=None, global_step=1.0E99):
    ...
    if isinstance(p, DiagonalGaussianVar):
        if isinstance(q, DiagonalGaussianVar):
            safe_qvar = q.var + EPSILON
            entropy_term = 0.5 * (1 + log2pi + tf.log(p.var))
            cross_entropy_term = 0.5 * (log2pi + tf.log(safe_qvar) + (p.var + (p.mean - q.mean)**2) / safe_qvar)           
            return tf.reduce_sum(cross_entropy_term - entropy_term)
        elif isinstance(q, DiagonalLaplaceVar):
            sigma = tf.sqrt(p.var)
            mu_ovr_sigma = p.mean / sigma
            tmp = 2 * bu.standard_gaussian(mu_ovr_sigma) + mu_ovr_sigma * tf.erf(mu_ovr_sigma * one_ovr_sqrt2)
            tmp *= sigma / q.b
