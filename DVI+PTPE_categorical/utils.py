...
def run_one_epoch(sess, data, model, metrics, train_op, hypers, dynamic_hypers):
    ...
    count = 0
    running_accuracy = running_logprob = running_brier = 0
    for batch in batched(data, hypers):
        ...
        running_logprob = \
            ...
            / new_count
        running_brier = \
            (count * running_brier + len(batch[0]) * result[0]['brier']) \
            / new_count
...
def train_valid_test(data, sess, model_and_metrics, train_op, hypers, verbose=True):
    ...
    for section in hypers['sections_to_run']:
        ...
        if verbose:
            ...
            print(' %s accuracy = %.4f | logL = %.4f | brier = %.4f' % (section, accuracies[section], 
                                                          summary[section][-1][-1]['running_logprob'],
                                                          summary[section][-1][-1]['running_brier']))
    if verbose:
        print()
