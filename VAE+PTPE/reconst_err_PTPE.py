...
latent_size = 2
model_2D = VAE_EP(input_size=784, hidden_size=500, latent_size=latent_size).to(device)
...
latent_size = 5
model_5D = VAE_EP(input_size=784, hidden_size=500, latent_size=latent_size).to(device)
checkpt  = torch.load(os.path.join(paras_dir, 'mnist_zdim' + str(latent_size) + '.pkl'))
model_5D.load_state_dict(checkpt['model_state_dict'])

latent_size = 10
model_10D = VAE_EP(input_size=784, hidden_size=500, latent_size=latent_size).to(device)
checkpt  = torch.load(os.path.join(paras_dir, 'mnist_zdim' + str(latent_size) + '.pkl'))
model_10D.load_state_dict(checkpt['model_state_dict'])

latent_size = 20
model_20D = VAE_EP(input_size=784, hidden_size=500, latent_size=latent_size).to(device)
checkpt  = torch.load(os.path.join(paras_dir, 'mnist_zdim' + str(latent_size) + '.pkl'))
...
with torch.no_grad():    
    ...
    print(f"mean: {np.mean(all_reconst_loss1)}")
    print(f"mean: {np.mean(all_reconst_loss2)}")
    print(f"mean: {np.mean(all_reconst_loss3)}")
    print(f"mean: {np.mean(all_reconst_loss4)}")    
