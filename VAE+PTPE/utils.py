...
def make_gif(imgs_dir, num_imgs, latent_size, t=100):
    imgs = []
    for i in range(num_imgs):
        img_name = '{}/samples_zdim{}-{}.png'.format(imgs_dir, latent_size, i)
        temp = Image.open(img_name)
        imgs.append(temp)
    imgs[0].save(fp=os.path.join(imgs_dir, f'samples_zdim{latent_size}-gif.gif'), save_all=True, append_images=imgs, duration=t, loop=0)
    print('GIF done!')
...
def plot_elbocurve(train_elbo, test_elbo, latent_size, save_dir):
    ...
    plt.title('MNIST, $N_z$={}'.format(latent_size))
