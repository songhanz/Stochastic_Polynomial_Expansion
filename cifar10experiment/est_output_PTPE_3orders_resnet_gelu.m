...
% propagate variance
[modelO3_mean, modelO3_cov] = PropCovResNet_GELU('ST3', stacks, net.Layers, input_mean, input_cov, reshaped_convW);

modelO3_mean = gather(modelO3_mean);
modelO3_cov  = gather(modelO3_cov);

%% Stochastic Taylor 2nd order
[modelO2_mean, modelO2_cov] = PropCovResNet_GELU('ST2', stacks, net.Layers, input_mean, input_cov, reshaped_convW);

modelO2_mean = gather(modelO2_mean);
modelO2_cov  = gather(modelO2_cov);

%% Stochastic Taylor 1st order (Stochastic linearization)
[modelO1_mean, modelO1_cov] = PropCovResNet_GELU('ST1', stacks, net.Layers, input_mean, input_cov, reshaped_convW);

modelO1_mean = gather(modelO1_mean);
modelO1_cov  = gather(modelO1_cov);

%% Jacobian Linearization
[modelJL_mean, modelJL_cov] = PropCovResNet_GELU('JL', stacks, net.Layers, input_mean, input_cov, reshaped_convW);
...
function [new_mean, new_cov] = PropCovResNet_GELU(type, stacks, layers, input_mean, input_cov, reshaped_convWs)
...
[new_mean, new_cov, ~]        = GELU(type, new_mean, new_cov, []);
...
[new_mean, new_cov, new_xcov] = GELU(type, new_mean, new_cov, new_xcov);        % if isequal(layers(3).Name, 'leakyrelu3');ii = 10; how_good_is_mean_var_esti(ii, new_cov, new_mean, layers(3), Var_all{ii}, Mean_all{ii}); end
...
function [new_mean, new_cov, new_xcov] = GELU(type, new_mean, new_cov, new_xcov)    
    ...
    if isequal(type, 'JL')
        ...
        w            = normcdf(z) + z .* normpdf(z);
    
        new_mean     = new_mean .* normcdf(new_mean);
        new_cov      = w .* new_cov .* w';
