...
% propagate variance
[modelO3_mean, modelO3_cov] = PropCovResNet_ReLU('ST3', stacks, net.Layers, input_mean, input_cov, reshaped_convW);

modelO3_mean = gather(modelO3_mean);
modelO3_cov  = gather(modelO3_cov);

%% Stochastic Taylor 2nd order
[modelO2_mean, modelO2_cov] = PropCovResNet_ReLU('ST2', stacks, net.Layers, input_mean, input_cov, reshaped_convW);

modelO2_mean = gather(modelO2_mean);
modelO2_cov  = gather(modelO2_cov);

%% Stochastic Taylor 1st order (Stochastic linearization)
[modelO1_mean, modelO1_cov] = PropCovResNet_ReLU('ST1', stacks, net.Layers, input_mean, input_cov, reshaped_convW);

modelO1_mean = gather(modelO1_mean);
modelO1_cov  = gather(modelO1_cov);

%% Jacobian Linearization
[modelJL_mean, modelJL_cov] = PropCovResNet_ReLU('JL', stacks, net.Layers, input_mean, input_cov, reshaped_convW);

modelJL_mean = gather(modelJL_mean);
modelJL_cov  = gather(modelJL_cov);

%% Frey & Hinton 1999
[modelfrey_mean, modelfrey_cov] = PropCovResNet_ReLU('frey', stacks, net.Layers, input_mean, input_cov, reshaped_convW);

modelfrey_mean = gather(modelfrey_mean);
modelfrey_cov  = gather(modelfrey_cov);


save([savedir 'predoutput/' netname '_' nonlin '_preddist_var' num2str(in_var) '.mat'], ...
     'modelO3_mean', 'modelO3_cov', ...
     'modelO2_mean', 'modelO2_cov', ...
     'modelO1_mean', 'modelO1_cov', ...
     'modelJL_mean', 'modelJL_cov',...
     'modelfrey_mean', 'modelfrey_cov');

end
end

%%
function [new_mean, new_cov] = PropCovResNet_ReLU(type, stacks, layers, input_mean, input_cov, reshaped_convWs)
...
[new_mean, new_cov, ~]        = ReLU(type, new_mean, new_cov, []);
...
[new_mean, new_cov, new_xcov] = ReLU(type, new_mean, new_cov, new_xcov);        % if isequal(layers(3).Name, 'leakyrelu3');ii = 10; how_good_is_mean_var_esti(ii, new_cov, new_mean, layers(3), Var_all{ii}, Mean_all{ii}); end
...
function [new_mean, new_cov, new_xcov] = Conv2D(layer, new_mean, new_cov, new_xcov, prod_direction, weight_reshape)
    ...
    if sum(padding)
        ...
        % Adding zeros into the covariance matrix will ruin the positive
...
function [new_mean, new_cov, new_xcov] = ReLU(type, new_mean, new_cov, new_xcov)    
    ...
    new_cov = set_min_var(new_cov, epsilon);
    % Jacobian linearization
    if isequal(type, 'JL')
        ...
        new_mean     = max(new_mean, 0); 
        new_cov      = new_cov .* idxsq;
        ...
        if ~isempty(new_xcov)
            ...
            new_xcov = idx .* new_xcov;
        else
            % new_xcov = [];
        end

    % Solution suggested by Frey and Hinton 1999
    elseif isequal(type, 'frey')
        ...
        new_var  = z .* A0 + s2 .* cdf - A0.^2;
        new_cov  = diag(new_var); % propagating only the variance
    
        if ~isempty(new_xcov)
            % propagating only the variance
            new_xcov = zeros(size(new_xcov));
        else
            % new_xcov = [];
        end

    else
        % only upto the third order stochastic Taylor expansion are implemented
        z            = double(reshape(new_mean, [], 1));    
        s2           = double(diag(new_cov));
        s            = sqrt(s2); 
        ...
        elseif isequal(type, 'ST3')
            ...
            A2           = 1/2 * pdf;
            A3           = -1/factorial(3) .* pdf .* z ./ s2;
            
            new_mean     = reshape(A0, size(new_mean)); 
    
            new_cov      = A1 .* new_cov .* A1' + ...
                           A2 .* (2 * new_cov.^2) .* A2'+ ...
                           A3 .* (6 * new_cov.^3 + 9 * s2 .* new_cov .* s2') .* A3';
