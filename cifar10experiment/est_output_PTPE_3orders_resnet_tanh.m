...
[modelJL_mean, modelJL_cov] = PropCovResNet_Tanh('JL', stacks, net.Layers, input_mean, input_cov, reshaped_convW);

modelJL_mean = gather(modelJL_mean);
modelJL_cov  = gather(modelJL_cov);

%% Wang et al
[modelwang_mean, modelwang_cov] = PropCovResNet_Tanh('wang', stacks, net.Layers, input_mean, input_cov, reshaped_convW);

modelwang_mean = gather(modelwang_mean);
modelwang_cov  = gather(modelwang_cov);


save([savedir 'predoutput/' netname '_' nonlin '_tiny_preddist_var' num2str(in_var) '.mat'], ...
     'modelO3_mean', 'modelO3_cov', ...
     'modelO2_mean', 'modelO2_cov', ...
     'modelO1_mean', 'modelO1_cov', ...
     'modelJL_mean', 'modelJL_cov',...
     'modelwang_mean', 'modelwang_cov');
...
function [new_mean, new_cov, new_xcov] = Tanh(type, new_mean, new_cov, new_xcov)   
    ...
    %gamma = [0.932509446848035,0.743885472858475,0.711503736783192,0.707185882493931,1.036267327797645,1.041272595334387,0.706179751346938,0.484137541329413,1.053317393402910,1.444605073000763];
